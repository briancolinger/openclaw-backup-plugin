...
describe("parseBackupConfig", () => {
  ...
  it("should warn but not throw when no destinations are configured", () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining("no destinations configured"),
    );
    spy.mockRestore();
  });
  ...
  it("should throw when raw is not an object", () => {
    for (const bad of ["string", null, 42, []]) {
      expect(() => parseBackupConfig(bad)).toThrow(
        "backup config must be a JSON object",
      );
    }
  });
  ...
  it("should throw when encrypt is not a boolean", () => {
    expect(() =>
      parseBackupConfig({ encrypt: "yes", destinations: { l: { path: "/" } } }),
    ).toThrow("config.encrypt must be a boolean");
  });
  ...
  it("should throw when schedule is not a valid cron expression", () => {
    ...
    expect(() =>
      parseBackupConfig({ schedule: 42, destinations: { l: { path: "/" } } }),
    ).toThrow("config.schedule must be a string");
  });
  ...
  it("should throw when destination configuration is invalid", () => {
    expect(() =>
      parseBackupConfig({ destinations: { broken: {} } }),
    ).toThrow('config.destinations.broken must have either "path" or "remote"');

    expect(() =>
      parseBackupConfig({ destinations: ["not", "an", "object"] }),
    ).toThrow("config.destinations must be an object");

    expect(() =>
      parseBackupConfig({ destinations: { local: { path: 123 } } }),
    ).toThrow("config.destinations.local.path must be a string");

    expect(() =>
      parseBackupConfig({ destinations: { cloud: { remote: true } } }),
    ).toThrow("config.destinations.cloud.remote must be a string");
  });
});
