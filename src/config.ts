...
function parseDestination(name: string, raw: unknown): DestinationConfig {
  ...
  if (pathVal === undefined && remoteVal === undefined) {
    throw new Error(
      `config.destinations.${name} must have either "path" or "remote"`,
    );
  }
...
function readConfigFile(resolved: string): unknown {
  try {
    ...
    if (cause instanceof Error) {
      throw new Error(
        `Failed to read openclaw config from ${resolved}: ${cause.message}`,
        { cause },
      );
    }
...
export function parseBackupConfig(raw: unknown): BackupConfig {
  ...
  const config: BackupConfig = {
    ...
    includeTranscripts: parseBoolean(
      raw["includeTranscripts"],
      "includeTranscripts",
      false,
    ),
    includePersistor: parseBoolean(
      raw["includePersistor"],
      "includePersistor",
      false,
    ),
